...
export const ProductCard: React.FC<ProductCardProps> = ({ stats }) => {
  ...
  return (
    <div
      ...
      className="group relative bg-white dark:bg-slate-900 hover:bg-slate-50 dark:hover:bg-slate-800/80 border border-slate-200 dark:border-slate-800 hover:border-emerald-500/50 rounded-3xl p-4 sm:p-5 flex flex-col justify-between transition-all duration-300 hover:shadow-xl hover:-translate-y-1 cursor-pointer shadow-sm"
    >
      {/* Top Section: Badges & Image */}
      <div>
        <div className="relative aspect-4/3 w-full bg-slate-100 dark:bg-slate-950/60 rounded-2xl overflow-hidden mb-3.5 border border-slate-200 dark:border-slate-800 flex items-center justify-center">
          {stats.imageUrl ? (
            <img
              src={stats.imageUrl}
        ...
        {/* Brand & Category */}
        <div className="flex items-center justify-between gap-2 text-[11px] font-medium text-slate-500 dark:text-slate-400 mb-1">
          <span className="uppercase tracking-wider text-emerald-600 dark:text-emerald-400 font-bold truncate">
            {stats.brand}
          </span>
          <span className="truncate bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded-md text-slate-600 dark:text-slate-300">
            {stats.category}
          </span>
        </div>

        {/* Product Title */}
        <h3 className="text-sm sm:text-base font-bold text-slate-900 dark:text-slate-100 line-clamp-2 leading-snug group-hover:text-emerald-500 transition-colors">
          {stats.productName}
        </h3>
        ...
        <div className="flex items-center gap-2 mt-2.5 text-xs">
          <div className="flex items-center gap-1 bg-amber-500/10 text-amber-600 dark:text-amber-400 px-2 py-0.5 rounded-lg border border-amber-500/20 font-semibold">
          ...
          {stats.totalRatings > 0 ? (
            <span className="text-[11px] text-slate-500 dark:text-slate-400">
              ...
              <span className="text-emerald-600 dark:text-emerald-400 font-medium">{stats.recommendationRate}% recomiendan</span>
            </span>
          ) : (
            <span className="text-[11px] text-slate-400 dark:text-slate-500 italic">Sé el primero en calificar</span>
          )}
        </div>
      </div>
      ...
      <div className="mt-4 pt-3.5 border-t border-slate-200 dark:border-slate-800">
          ...
          <div>
            <div className="flex items-baseline justify-between gap-2">
              ...
              {stats.savingsVsHighest > 0 && (
                <div className="flex items-center gap-1 bg-emerald-500/10 border border-emerald-500/20 text-emerald-600 dark:text-emerald-400 text-xs font-bold px-2 py-1 rounded-xl">
        ...
        <div className="grid grid-cols-3 gap-1.5 mt-3.5">
          <button
            type="button"
            onClick={handleOpenDetail}
            className="flex items-center justify-center gap-1 py-2 px-2 bg-slate-100 dark:bg-slate-800 hover:bg-emerald-600 hover:text-white dark:hover:bg-emerald-600 text-slate-700 dark:text-slate-200 rounded-xl text-xs font-semibold transition-all border border-slate-200 dark:border-slate-700"
          ...
          <button
            ...
            className="flex items-center justify-center gap-1 py-2 px-2 bg-emerald-500/10 hover:bg-emerald-500 text-emerald-600 dark:text-emerald-400 hover:text-slate-950 rounded-xl text-xs font-semibold transition-all border border-emerald-500/30"
          ...
          <button
            type="button"
            onClick={handleRate}
            className="flex items-center justify-center gap-1 py-2 px-2 bg-purple-500/10 hover:bg-purple-500 text-purple-600 dark:text-purple-400 hover:text-white rounded-xl text-xs font-semibold transition-all border border-purple-500/30"
