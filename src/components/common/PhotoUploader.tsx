...
export const PhotoUploader: React.FC<PhotoUploaderProps> = ({
  ...
  const currentVal = value !== undefined ? value : initialUrl;
  const [previewUrl, setPreviewUrl] = useState<string | undefined>(currentVal);
    ...
    <div className="w-full space-y-2">
      ...
      <div
        ...
        className={`relative border-2 border-dashed rounded-2xl p-4 transition-all duration-200 cursor-pointer overflow-hidden group flex flex-col items-center justify-center min-h-[140px] text-center ${
          previewUrl
            ? 'border-emerald-500/40 bg-emerald-50/50 dark:bg-slate-900/60'
            : 'border-slate-300 dark:border-slate-700 hover:border-emerald-500/50 bg-slate-50 dark:bg-slate-900/40 hover:bg-slate-100 dark:hover:bg-slate-900/80'
        }`}
      >
        <input
          ref={fileInputRef}
        ...
        {isUploading ? (
          ...
          <div className="relative w-full h-36 flex items-center justify-center">
            ...
            <button
              ...
              className="absolute top-2 right-2 p-1.5 bg-rose-600/90 hover:bg-rose-500 text-white rounded-full shadow-lg transition-colors cursor-pointer"
              title="Eliminar foto"
            >
              <X className="w-4 h-4" />
            </button>
            <div className="absolute bottom-2 left-1/2 -translate-x-1/2 bg-slate-900/80 backdrop-blur-sm text-slate-100 text-[11px] px-2.5 py-0.5 rounded-full border border-slate-700 flex items-center gap-1">
          ...
          <div className="flex flex-col items-center gap-2 py-2">
            <div className="w-12 h-12 rounded-2xl bg-emerald-100 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20 text-emerald-600 dark:text-emerald-400 flex items-center justify-center group-hover:scale-110 transition-transform">
